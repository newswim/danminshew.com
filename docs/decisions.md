...
| D-019 | 2026-07-18 | Mark rendered in beads, type set upright (amends D-016/D-018): the stroke tree read too thick and cross-like, so the same seed-55 skeleton is now drawn as touching dots sized by branch depth ("beaded", Dan's pick over stipple and tip-dots). OG name drops the italic — upright Iowan Old Style — after outside feedback that the italic read funereal | accepted |
| D-020 | 2026-07-18 | Stipple over beads (supersedes the treatment half of D-019; upright type stands): on reflection Dan prefers the separated-dot rendering — same skeleton and dot sizing, gapK ~3 leaves air between dots. Known trade-off, accepted with eyes open: the airier mark is fainter at 16px favicon size | accepted |
